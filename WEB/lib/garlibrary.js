...
gar.getButton=function (e) {
    if(this.doc.implementation.hasFeature('MouseEvents','2.0'))return e.button;
    else
...
gar.imitClick=function (ele) {
    ...
    event.initMouseEvent('click',true,true,this.doc.defaultView,0,0,0,0,0,false,false,false,false,0,null);
...
function getEl(cssSelector) {
    return gar.doc.querySelector(cssSelector);
}
function getEls(cssSelector){
    return gar.doc.querySelectorAll(cssSelector);
}
...
gar.getViewpoint=function () {
    ...
    //attribute of this.doc.compatMode is not supported by lower Safari 3.1
    if(this.doc.compatMode=='BackCompat')
        return{
            width:gar.doc.body.clientWidth,
            height:gar.doc.body.clientHeight
        };
    else return{
        width:gar.doc.documentElement.Width,
        height:gar.doc.documentElement.clientHeight
    };
};

/**
 * get this.doc total height n' width
 */
gar.getDocHeight=function (){
    //IE backCompat: this.doc.body
    return this.doc.compatMode=='CSS1Compat'?Math.max(this.docdocumentElement.scrollHeight,this.doc.documentElement.clientHeight):Math.max(this.doc.body.scrollHeight,this.doc.body.clientHeight);
};
gar.getDocWidth=function (){
    return this.doc.compatMode=='CSS1Compat'?Math.max(this.doc.documentElement.scrollWidth,this.doc.documentElement.clientWidth):Math.max(this.doc.body.scrollWidth,this.doc.body.clientWidth);
};
...
gar.getBoundingClientRect=function (ele){
    var scrollTop=this.doc.documentElement.scrollTop,
        scrollLeft=this.doc.documentElement.scrollLeft;
...
gar.getStyle=function(ele,css,pseudoEleInfo){
    if(this.doc.defaultView)this.getStyle=function (ele,css, pseudoEleInfo) {
...
gar.cookieUtil={
    get:function (name) {
        var cName=encodeURIComponent(name)+'=',
            cStart=gar.doc.cookie.indexOf(cName),
            cVal=null;
        if(cStart>-1){
            var cEnd=gar.doc.cookie.indexOf(';',cStart);
            if(cEnd===-1)cEnd=gar.doc.cookie.length;
            cVal=decodeURIComponent(gar.doc.cookie.substring(cStart+cName.length,cEnd));
        }
        return cVal;
    },
...
gar.subCookieUtil={
    ...
    getAll:function (name) {
        var cN=encodeURIComponent(name)+'=',
            cStart=gar.doc.cookie.indexOf(cN),
            cVal=null,
            cEnd,subCookie,i,parts,result={};
        if(cStart>-1){
            cEnd=gar.doc.cookie.indexOf(';',cStart);
            if(cEnd===-1)cEnd=gar.doc.cookie.length;
            cVal=gar.doc.cookie.substring(cStart+cN.length,cEnd);
            if(cVal.length>0){
...
gar.getFullPageWH=function (wOrh) {
    var pageWidth=this.doc.innerWidth,pageHeight=this.doc.innerHeight;
    if(typeof pageWidth!='number'){
        if(this.doc.compatMode=='CSS1Compat'){
            pageWidth=this.doc.documentElement.clientWidth;
            pageHeight=this.doc.documentElement.clientHeight;
        }else{
            pageWidth=this.doc.body.clientWidth;
            pageHeight=this.doc.body.clientHeight;
        }
    }
...
gar.isDOM1=function () {
    return this.isDOM1=!!(this.doc.getElementById&&this.doc.createElement&&this.doc.getElementsByTagName);
};
            ...
            handler=function (e) {
                ...
                if(e.touches.lengt===1){
                    switch (e.type){
                        ...
                        case 'touchend':
                            gar.removeHandler(gar.doc,'touchstart',handler,false);
                            gar.removeHandler(gar.doc,'touchmove',handler,false);
                            gar.removeHandler(gar.doc,'touchend',handler,false);
                    }
                }
            };
        gar.addHandler(this.doc,'touchstart',handler,false);
        gar.addHandler(this.doc,'touchmove',handler,false);
        gar.addHandler(this.doc,'touchend',handler,false);
    }
    return handler;
};
